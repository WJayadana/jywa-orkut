...
/**
 * JywaOrkut API wrapper for Indonesian QRIS payment system.
 *
 ...
 * ```
 */
export default class JywaOrkut {
  ...
  async getOTP(): Promise<any> {
    try {
      const payload = qs.stringify({
        ...
        ...JywaOrkut.OK_CONSTANTS,
      });

      const response: AxiosResponse = await axios.post(
        JywaOrkut.OK_LOGIN_ENDPOINT,
        payload,
        {
          headers: {
            ...JywaOrkut.OK_HEADERS,
            "User-Agent": "okhttp/4.12.0",
          },
        },
      );
  ...
  async getQRISHistory(
    ...
    try {
      ...
      const payload = {
        app_reg_id: JywaOrkut.OK_CONSTANTS.app_reg_id,
        phone_uuid: JywaOrkut.OK_CONSTANTS.phone_uuid,
        phone_model: JywaOrkut.OK_CONSTANTS.phone_model,
        [`requests[${historyType}][keterangan]`]: options.keterangan || "",
        [`requests[${historyType}][jumlah]`]: options.jumlah || "",
        request_time: timestamp,
        phone_android_version: JywaOrkut.OK_CONSTANTS.phone_android_version,
        app_version_code: JywaOrkut.OK_CONSTANTS.app_version_code,
        auth_username: this.username,
        [`requests[${historyType}][page]`]: options.page || "1",
        auth_token: this.token,
        app_version_name: JywaOrkut.OK_CONSTANTS.app_version_name,
        ui_mode: JywaOrkut.OK_CONSTANTS.ui_mode,
